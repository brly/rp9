...
impl UncompressedHeader {
    fn new() -> UncompressedHeader {
        UncompressedHeader {
            LastFrameType: FrameType::KEY_FRAME,
...
impl Context {
    ...
    fn f(&mut self, value: u16, width: u16) {
        self.bs.push(BitStream::new(value, width));
    }
}

// 6.1 Frame syntax
fn frame(context: &mut Context/*sz: u32*/) {
    // startBitPos = get_position()
    uncompressed_header(context);
    // trailing_bits()
...
fn uncompressed_header(context: &mut Context) {
    context.f(2, 2); // frame_marker     # f(2)
    context.f(0, 1); // profile_low_bit  # f(1)
    context.f(0, 1); // profile_high_bit # f(1)
    // Profile = (profile_high_bit << 1) + profile_low_bit
    // if (Profile == 3)
    //   reserved_zero  # f(1)
    // 存在するフレームを見せるかどうかのフラグ ?
    context.f(0, 1); // show_existing_frame  # f(1)
    ...
    context.f(0, 1); // とりあえず固定で KEY_FRAME // frame_type       # f(1)
    context.f(1, 1); // 説明がない // show_frame       # f(1)
    context.f(0, 1); // error_resilient_mode # f(1)
    if true {// if ( frame_type == KEY_FRAME ) {
        frame_sync_code(context); //   frame_sync_code()
        color_config(context); //   color_config()
    ...
    if true { // if ( error_resilient_mode == 0 ) {
        context.f(0, 1); // 計算した確率をストアするか捨てるかどうか. 0 で捨てる  refresh_frame_context # f(1)
        context.f(0, 1); // マルチスライスと同じ？ frame_parallel_decoding_mode # f(1)
    } else { // } else {
        //   refresh_frame_context = 0
        //   frame_parallel_decoding_mode = 1
    }
    context.f(0, 2); // メモリ上にあるとされる 0~3 までのいずれかのテーブルを指定 // frame_context_idx # f(2)
    if true { // if ( FrameIsIntra || error_resilient_mode ) {
        // 状態をかなり初期化するらしい // setup_past_independence()
    ...
    loop_filter_params(context);
    quantization_params(context);
    segmentation_params(context);
    tile_info(context);
    // compressed ヘッダサイズを出す // header_size_in_bytes # f(16)
}

// 6.2.1 Frame sync syntax
fn frame_sync_code(context: &mut Context) {
    context.f(0x49, 8); // frame_sync_byte_0    # f(8)
    context.f(0x83, 8); // frame_sync_byte_1    # f(8)
    context.f(0x42, 8); // frame_sync_byte_2    # f(8)
}

// 6.2.2 Color config syntax
fn color_config(context: &mut Context) {
    // if ( Profile >= 2 ) {
    ...
    //   BitDepth = 8
    // }
    context.f(1, 3); // color_space  # f(3)
    if true {// if ( color_space != CS_RGB ) {
        context.f(1, 1); //   color_range    # f(1)
...
fn frame_size(context: &mut Context) {
    context.f(351, 16); // frame_width_minus_1  # f(16)
    context.f(287, 16); // frame_height_minus_1 # f(16)
...
fn render_size(context: &mut Context) {
    context.f(0, 1); // よくわかってない TODO // render_and_frame_size_different  # f(1)
...
fn loop_filter_params(context: &mut Context) {
    context.f(0, 6); // 0 だと invoke しなさそう // loop_filter_level # f(6)
    context.f(0, 3); // loop_filter_sharpness        # f(3)
    context.f(0, 1); // loop_filter_delta_enabled    # f(1)
...
fn quantization_params(context: &mut Context) {
    context.f(1, 8); // base_q_idx   # f(8)
    read_delta_q(context); // delta_q_y_dc = read_delta_q()
    read_delta_q(context); // delta_q_uv_dc = read_delta_q()
    read_delta_q(context); // delta_q_uv_ac = read_delta_q()
    // Lossless = base_q_idx == 0 && delta_q_y_dc == 0
    //                              && delta_q_uv_dc == 0 && delta_q_uv_ac == 0
}

// 6.2.10 Delta quantizer syntax
fn read_delta_q(context: &mut Context) {
    context.f(0, 1); // delta_coded  # f(1)
    // if ( delta_coded ) {
    //   delta_q    # f(4)
    // } else {
    //   delta_q = 0
...
fn segmentation_params(context: &mut Context) {
    context.f(0, 1); // segmentation_enabled # f(1)
...
fn tile_info(context: &mut Context) {
    // TODO Tile の計算をしている. ここでこけそう.
